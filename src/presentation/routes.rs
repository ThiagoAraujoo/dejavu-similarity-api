...
pub fn create_routes() -> Router {
    ...
    let transcription_service = match TranscriptionService::new() {
        Ok(service) => service,
        Err(e) => {
            tracing::error!("Failed to initialize transcription service: {}", e);
            tracing::warn!("Transcription endpoints will not be available");
            return Router::new()
    ...
    let rest_state = Arc::new(api::transcription::TranscriptionRestState {
    ...
    Router::new()
        ...
        .route("/transcription", post(api::transcription::upload_file))
        .with_state(rest_state.clone())
        // WebSocket for status updates
        .route("/transcription", get(websocket::transcription::status_websocket_handler))
